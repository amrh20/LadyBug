...
@Component({
  selector: "app-create-animal-fodder",
  templateUrl: "./create-animal-fodder.component.html",
  styleUrls: ["./create-animal-fodder.component.scss"],
})
export class CreateAnimalFodderComponent implements OnInit {
  form = new FormGroup(
    {
  ...
  sendData = () => {
    ...
    if (this.form.valid) {
      this.loading = true;
      if (this.isEdit) {
        this._crud.put(`animal_fodder_sources/${this.idEdit}`, data).subscribe(
          (res: any) => {
            this.loading = false;
            this.isSubmit = false;
        ...
        this._crud.post(`animal_fodder_sources`, data).subscribe(
          (res: any) => {
            this.loading = false;
            this.isSubmit = false;
  ...
  goBack = () => {
    this.route.navigate(["/admin/farmer/animal_fodder_sources"]);
  };
}
