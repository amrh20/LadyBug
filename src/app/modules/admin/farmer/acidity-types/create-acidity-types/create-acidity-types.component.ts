...
@Component({
  selector: "app-create-acidity-types",
  templateUrl: "./create-acidity-types.component.html",
  styleUrls: ["./create-acidity-types.component.scss"],
})
export class CreateAcidityTypesComponent implements OnInit {
  form = new FormGroup(
    {
  ...
  sendData = () => {
    ...
    if (this.form.valid) {
      this.loading = true;
      if (this.isEdit) {
        this._crud.put(`acidity_types/${this.idEdit}`, data).subscribe(
          (res: any) => {
            this.loading = false;
            this.isSubmit = false;
        ...
        this._crud.post(`acidity_types`, data).subscribe(
          (res: any) => {
            this.loading = false;
            this.isSubmit = false;
  ...
  goBack = () => {
    this.route.navigate(["/admin/farmer/acidity_types"]);
  };
}
