...
@Component({
  selector: "app-create-seedling-sources",
  templateUrl: "./create-seedling-sources.component.html",
  styleUrls: ["./create-seedling-sources.component.scss"],
})
export class CreateSeedlingSourcesComponent implements OnInit {
  form = new FormGroup(
    {
  ...
  sendData = () => {
    ...
    if (this.form.valid) {
      this.loading = true;
      if (this.isEdit) {
        this._crud.put(`seedling_sources/${this.idEdit}`, data).subscribe(
          (res: any) => {
            this.loading = false;
            this.isSubmit = false;
        ...
        this._crud.post(`seedling_sources`, data).subscribe(
          (res: any) => {
            this.loading = false;
            this.isSubmit = false;
  ...
  goBack = () => {
    this.route.navigate(["/admin/farmer/seedling_sources"]);
  };
}
