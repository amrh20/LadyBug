...
export class IrrigationMethodsTableComponent implements OnInit {
  ...
  getUsers = () => {
    this._CrudRequestsService
      .get("irrigation_ways" + `?page=${ this.current }&perPage=10`)
      .subscribe((data: any) => {
  ...
  reset() {
    ...
    this._CrudRequestsService
      .get(
        `irrigation_ways?page=${ this.current }&perPage=10`
      )
      .subscribe((data: any) => {
        this.DataTable = data.data.all;
        this.last = data.data.meta.pagesCount;
  ...
  deleteItem = (id: any) => {
    Swal.fire({
      ...
      if (val.isConfirmed) {
        this._CrudRequestsService.delete("irrigation_ways/" + id).subscribe(
          (res: any) => {
